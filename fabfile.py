...
def start_gunicorn():
    with cd(env.app_root):
        run('%(virtualenv)s/bin/gunicorn --pid=gunicorn.pid --daemon --workers=3 --access-logfile=devincachu_access.log --error-logfile=devincachu_error.log --bind=127.0.0.1:8989 devincachu.wsgi' % env)
